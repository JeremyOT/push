...
- Implemented Signal CLI JSON-RPC daemon integration. Added `--signal-server` and `--signal-address` CLI flags to enable the integration. Added a `/signal` command to activate a push session to respond to Signal messages (the active session saves status confirmation messages as `Kind = "status"` to display in-line in the chat window). Implemented connection to the native `signal-cli` HTTP daemon JSON-RPC interface: handles incoming messages by listening to the real-time Server-Sent Events (SSE) events stream (`GET /api/v1/events`) and issues outbound messages, reactions (adding/removing 👀 and ✅), and typing states via JSON-RPC POST payloads to `/api/v1/rpc`. Enforced a safety constraint where push only processes incoming Signal messages whose sender matches `--signal-address` (the admin/allowed sender's phone number). Enhanced the daemon client with full fault tolerance, implementing a thread-safe connection state logger that tracks status, supports daemon restarts/outages, and automatically reconnects/re-establishes the SSE stream in the background. Fixed the interactive CLI service tmux client forwarding to skip typing `/signal` commands into the active session's shell/input buffer, preventing them from prefixing the next user message. Added the `TestSignalIntegration` unit test verifying the SSE events stream and JSON-RPC methods.

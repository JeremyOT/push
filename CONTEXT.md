...
- Added client-side write-in choice support in `static/chat-app.jsx`: when a user selects a "Write-in..." option from an interactive question card, the Web UI prompts them for text input, sends the option index, and then sends the write-in text response sequentially after a 500ms delay.
- Fixed client-side write-in option detection in `static/chat-app.jsx` to be case-insensitive, support options both with and without hyphens (e.g. "write-in" and "write in"), and correctly handle option values represented as plain strings instead of objects.

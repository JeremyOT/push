...
- Implemented automatic image scraping, downscaling, embedding, and horizontal carousel rendering. When a message (message or detailed_message) contains image file names, relative/absolute paths, or URLs (e.g. in text, markdown, or HTML format), the backend automatically scrapes the image. If the image is >256KB, it downscales it (preserving aspect ratio, up to 1024px maximum dimension) and embeds it as a base64 PNG data URL in the database. Added support in the frontend React application (`chat-app.jsx` and `chat-messages.jsx`) to map these embedded images and render them at the end of user and agent messages in a horizontally scrollable carousel. Integrated a lightbox/modal viewer to view full resolution images on click. Added the `TestImageScraper` unit test.
